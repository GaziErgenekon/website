  ...
  return (
    <div className="container my-16">
      <div className="grid xls:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          <div key={i} className={`col-center gap-2 inset-shadow-sm p-6 rounded-lg border ${COLOR_VARIANTS[stat.color]} bg-radial-[at_50%_75%]  to-100%`}>
            <div className="font-semibold text-3xl xl:text-4xl text-gradient-br from-foreground to-foreground/20  ">
              {stat.title}
            </div>
            <p className="text-lg font-semibold">{tx(stat.value)}</p>
          </div>
        ))}
      </div>
    </div>
